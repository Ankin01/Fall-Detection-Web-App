...
def save_instances(image, boxes, masks, class_ids, class_names,
                      ...
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
                      colors=None, captions=None,pathOut=None,img_name=None):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    masks: [height, width, num_instances]
    class_ids: [num_instances]
    ...
    for i in range(N):
        ...
        for verts in contours:
            ...
            ax.add_patch(p)
    ax.imshow(masked_image.astype(np.uint8))
    plt.savefig(os.path.join(pathOut, img_name),bbox_inches='tight', pad_inches=-0.5,orientation= 'landscape')

    if auto_show:         
        #plt.imsave(,masked_image.astype(np.uint8)) os.path.join(pathOut, img_name)
        plt.savefig('/home/andmerc/Thesis/Project/ProjectTwo/Output/frame_1.jpg',bbox_inches='tight', pad_inches=-0.5,orientation= 'landscape')
...
def save_image(image, imagee_name, boxes, masks, class_ids, scores, class_names, filter_classs_names=None,
               scores_thresh=0.1, save_dir=None, mode=0):
    """
        image: image array
        imagee_name: image name
        boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    ...
    if mode == 3:
        masked_image.save(os.path.join(save_dir, '%s.jpg' % (imagee_name)))
        return
